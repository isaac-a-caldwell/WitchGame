...
BattleManager.initMembers = function() {
    ...
    this._preservedSystem = new Game_System();
    this._preservedParty = new Game_Party();
    this._preservedTroop = new Game_Troop();
    this._preservedTroop = new Game_Actors();
};
...
BattleManager.onEncounter = function() {
    this._preemptive = false;//(Math.random() < this.ratePreemptive());
    this._surprise = false;//(Math.random() < this.rateSurprise() && !this._preemptive);
};
...
BattleManager.startBattle = function() {
    ...
    this._preservedSystem = _.cloneDeep($gameSystem);//JSON.parse(JSON.stringify($gameSystem));
    this._preservedParty = _.cloneDeep($gameParty);//JSON.parse(JSON.stringify($gameParty));
    this._preservedTroop = _.cloneDeep($gameTroop);//JSON.parse(JSON.stringify($gameTroop));
    this._preservedActors = _.cloneDeep($gameActors);
...
BattleManager.restartBattle = function() {
    const _ = require('./js/libs/lodash.js');

    $gameSystem = _.cloneDeep(this._preservedSystem);
    $gameParty = _.cloneDeep(this._preservedParty);
    $gameTroop = _.cloneDeep(this._preservedTroop);
    $gameActors = _.cloneDeep(this._preservedActors);
...
BattleManager.getPreservedParty = function() {
    return this._preservedParty;
}

BattleManager.getPreservedSystem = function() {
    return this._preservedSystem;
}

BattleManager.getPreservedActors = function() {
    return this._preservedActors;
}

BattleManager.getPreservedTroop = function() {
    return this._preservedTroop;
}
...
BattleManager.processEscape = function() {
    ...
    SoundManager.playEscape();
    var success = this._canEscape;//this._preemptive ? true : (Math.random() < this._escapeRatio);
    if (success) {
...
BattleManager.shouldRestarto = false;
...
BattleManager.shouldRestart = function() {
    if ($gameParty && $gameParty.inBattle())
    {
        if (Input._latestButton === 'restartBattle')
        {
            ...
            if( Input._pressedTime >= 70)
            {
                Input._pressedTime = 0;
                this.shouldRestarto = false;
