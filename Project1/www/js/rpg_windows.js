...
Window_EquipStatus.prototype.refresh = function() {
    ...
    if (this._actor) {
        ...
        var skips = 0;
        for (var i = 0; i < 6; i++) {
            if (TextManager.param(i+2) != "")
            {
                this.drawItem(0, this.lineHeight() * (1 + (i-skips)), 2 + i);
            }
            else
            {
                skips++;
...
Window_Status.prototype.drawBlock2 = function(y) {
    ...
    this.drawParameters(432, y);

};

Window_Status.prototype.drawBlock3 = function(y) {
    this.drawEquipments(252, y);
};
...
Window_Status.prototype.drawEquipments = function(x, y) {
    ...
    for (var i = 0; i < count; i++) {
        if (equips[i] != null)
        {
            this.drawItemName(equips[i], x, y + this.lineHeight() * (skippedLine ? i-1: i));
        }
        else
        {
            skippedLine = true;
...
Window_Message.prototype.updatePlacement = function() {
    ...
    this.y = this._positionType * (Graphics.boxHeight - this.height - 18) / 2;
...
Window_BattleLog.prototype.startAction = function(subject, action, targets) {
    ...
    this.displayAction(subject, item, targets.clone());
};
...
Window_BattleLog.prototype.displayActionResults = function(subject, target, action) {
    if (target.result().used) {
        this.push('pushBaseLine');
        this.displayCritical(target);
        this.push('popupDamage', target);
...
Window_BattleLog.prototype.displayFailure = function(target, action) {
    if (target.result().isHit() && !target.result().success && action._item.meta.skipfail == "false") {
...
Window_PartyCommand.prototype.makeCommandList = function() {
    ...
    this.addCommand('Restart', 'restart');
};
...
// Window_ResetGauge
//
// The window for displaying the reset gauge
function Window_ResetGauge() {
    this.initialize.apply(this, arguments);
}

Window_ResetGauge.prototype = Object.create(Window_Selectable.prototype);
Window_ResetGauge.prototype.constructor = Window_ResetGauge;

Window_ResetGauge.prototype.initialize = function() {
    var width = this.windowWidth();
    var height = this.windowHeight();
    var x = Graphics.boxWidth - width;
    var y = Graphics.boxHeight - height;
    Window_Selectable.prototype.initialize.call(this, x, y, 400, this.fittingHeight(1));
    this.refresh();
    this.hide();
};

Window_ResetGauge.prototype.windowWidth = function() {
    return Graphics.boxWidth - 200;
};

Window_ResetGauge.prototype.windowHeight = function() {
    return this.fittingHeight(8);
};

Window_ResetGauge.prototype.refresh = function(shouldDraw) {
    this.contents.clear();
    if (shouldDraw)
    {
        this.show();
        this.drawRestartGauge(0, 0, 300);
    }
    else
    {
        this.hide();
...
Window_ResetGauge.prototype.drawRestartGauge = function(x, y, width) {
    width = width || 300;
    var color1 = this.tpGaugeColor1();
    var color2 = this.tpGaugeColor2();
    this.drawGauge(x, y, width, Input._pressedTime/70, color1, color2);
    //this.changeTextColor(this.systemColor());
    this.drawText("Restarting battle...", x, y-6, 200);
    this.drawText(Math.round((Input._pressedTime/70)*100) + "%", x + width - 64, y-6, 64, 'right');
};
