...
Sprite_Actor.prototype.setActorHomeFrontView = function(index) {
    ...
    if (Yanfly.Param.BSWAdjustCol) {
      var size = ($gameParty.battleMembers().length == 1 ? 3 : $gameParty.battleMembers().length);
    } else {
...
Window_BattleStatus.prototype.maxCols = function() {
    if (Yanfly.Param.BSWAdjustCol) {
      return this.maxItems() == 1 ? 3: this.maxItems();
    } else {
...
Window_BattleStatus.prototype.itemWidth = function() {
    return this.contents.width / (this.maxCols() == 1 ? 3 : this.maxCols());
};
...
Window_BattleStatus.prototype.drawItem = function(index) {
  var index2 = index;
  if (index === 0 && Yanfly.Param.BSWAdjustCol === true && $gameParty.battleMembers().length == 1)
  {
    index2 = 1;
  }
    var actor = $gameParty.battleMembers()[index];
    this.drawBasicArea(this.basicAreaRect(index2), actor);
    this.drawGaugeArea(this.gaugeAreaRect(index2), actor);
    this.drawStateArea(this.basicAreaRect(index2), actor);
};
