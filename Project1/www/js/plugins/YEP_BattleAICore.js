...
AIManager.initialCheck = function(skillId) {
  ...
  var x = this.battler().meetsSkillConditions($dataSkills[skillId]);
  return x;
};
...
AIManager.setProperTarget = function(group) {
    ...
    var action = this.action();
    var randomTarget = group[0];//group[Math.floor(Math.random() * group.length)];
...
AIManager.setHighestHpRateTarget = function(group) {
    var maintarget = group[0];//Math.floor(Math.random() * group.length)];
...
AIManager.setRandomTarget = function(group) {
    ...
    this.action().setTarget(0);//target.index())
};
